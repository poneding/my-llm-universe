...
def fetch_repo_readme(org_name, repo_name, token, export_dir):
    ...
    if response.status_code == 200:
        ...
        loguru.logger.error(
            f"Error fetching README for {repo_name}: {response.status_code}"
        )
        loguru.logger.error(response.text)
...
if __name__ == "__main__":
    ...
    # 配置 export_dir
    export_dir = "knowledge_base/datawhale/readme_db"  # 请替换为实际的目录路径
    # 获取仓库列表
    repos = get_repos(org_name, GH_TOKEN, export_dir)
    # 打印仓库名称
    if repos:
        for repo in repos:
            repo_name = repo["name"]
            # 拉取每个仓库的README
            fetch_repo_readme(org_name, repo_name, GH_TOKEN, export_dir)
    # 清理临时文件夹
